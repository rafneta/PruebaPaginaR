...
Para esta parte se instalo el paquete *covidMex* con las instrucciones de la siguiente [página](https://github.com/pablorm296/covidMex), este paquete permite una adquisión de datos preprocesados.
...
Para tener los datos a disposición, se utilizo código como el siguiente en donde se leyeron los datos el 10 de junio y estos se guardaron para tener acceso posterior a esta fecha. 
...
Observamos que la Ciudad de México tiene el mayor número de casos confirmados acumulados con $40\,117$, seguido del Estado de México con $13\,879$ y por Baja Califronia con $6\,585$. Cabe destacar que el Estado de México y la ciudad de Mexico ocupan la primer y segunda posición en número de habitantes del país, lo cual podría explicar la cantidad de casos confirmados acumulados, por otro lado la densidad de población en la Ciudad e México es la primera y la del Estado de México la segunda de la nación, podríamos plantear al siguiente pregunta ¿la densidad de la entidad federativa explica el número de casos confirmados?, de manera intuitiva parece que no, pues en ese caso Baja California, y otros tantos estados, no figurarian en las primeras posiciones de casos confirmados ^[Aquí podría plantearse de manera formal un problema de regresión, para confirmar o desechar tal conjetura] 

Probablemente la _movilidad_ que hay en estas ciudades o la _cercania_ con otras entidades que presenten _grandes_ cantidades de casos confimados podría explicar estos resultados, por el momento es una hipótesis que en un trabajo futuro sería bueno responder.  



Otras preguntas que se puede hacer: ¿la edad de lo individuos influye en la suceptibilidad para contraer el enfermedad?, ¿Un determinado sexo es más propenso a contrar la enfermedad que el otro?, si un determinado sexo es más propenso a contraer la enfermedad ¿esta propensión es independiente de la edad?, la Figura \@ref(fig:casosedad)  muestra los casos confirmados agrupados por edad en intervalos de 10 años y la Figura \@ref(fig:casosedadsexo) muestra los casos confirmados agrupados por edad y sexo en intervalos de 10 años.
...
confirmados%>%
  ...
  hc_xAxis(title=list(text="Intervalos de 10 años"))%>%
    hc_exporting(enabled = TRUE,
               ...
               buttons = list(contextButton = list(menuItems = myMenuItems)))
```
Estás últimas gráficas pueden ayudar a tener una intuición sobre las respuestas a las preguntas previamente planteadas. Pare precisar las respuestas se pueden platear modelos econónometicos, en un futuro se espera responder a estas preguntas.

Se pueden plantear más preguntas que pueden ser de intéres para ayudar a la prevención en población vulnerable: ¿Los casos confirmados presentan alguna enfermedad respiratoria?, ¿Los casos confirmados presentan diabetes?, ¿Los casos confirmados presentan obesidad?, ¿Los casos confirmados presentan hipertención?, etc., para poder responder estas preguntas es necesario tener datos al respecto, algunas preguntas podrían ser importantes pero si no se cuenta con la información no será posible intenrtar responderlas de forma correcta. 
...
Una fuente oficial de información son los [Datos Abiertos](https://www.gob.mx/salud/documentos/datos-abiertos-152127) proporcionados por la Dirección General de Epidemiología. Estos datos que se actualiza a diario, en el caso estudiado se toman los del 10 de junio de 2020, se ofrecen en un archivo zip que contiene la base de datos en formato csv, y 2 archivo xls sobre la descripción de las variables y los valores que estás toman.  El archico descriptores da información de las 35 variables, entre las que se pueden encontrar RESULTADO, FECHA_ACTUALIZACION, TABAQUISMO, NEUMONIA, SEXO, EDAD, etc., la Figura \@ref(fig:tabladescriptores) muestra este archivo, se puede navegar sobre esta tabla y buscar información epecifica.
...
```
Si se quiere saber los valores que las variables toman y el significado de cada valor, se tiene que consultar el catalogo, cada variable se encuentra en hojas distintas cuyo número tiene que encontrarse, en nuestro caso nos interesa saber los valores de la varibale RESULTADO, así la hoja de interés es la $7$, en la Figura \@ref(fig:tablacatalogo) se muestra esta información 
...
En al Figura \@ref(fig:seriesmexico) se muestra la evolución en el tiempo de los casos confirmados *acumulados*^[nos referimos a los *casos confirmados acumulados* como *casos confirmados*] en México para cada entidad federativa. 
...
La Ciudad de México, Estado de México y Baja California han sido las ciudades que durante cada día se han colocado en los primeros 3 lugares nacionales, en particular se habia mostrado en la Figura \@ref(fig:casosestado) para el corte al día del 10 de junio 
...
Para verificar si los datos de la base de  [mariorz](https://github.com/mariorz/covid19-mx-time-series) concuerdan con los datos de la Dirección General de Epidemiología, de la última se calculo el acumulado nacional de casos confirmado al 10 de junio de 2020, lo cual es consistente con el dato $129\,184$ de la base utilizada
...
Los casos confimados acumulados por estado al 10 de junio pueden observarse en el sigueinte mapa
